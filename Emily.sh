...
Emily_menu(){ 
    case $(echo "$menu" | awk '{print $1'}) in
	 ...
	 "whats app " | status ) if [[ ! -d /sdcard/Emily/Whatsapp_statuses ]]; then
	 			      cd /sdcard/Emily/
				      mkdir Whatsapp_statuses
				      cd ->/dev/null
				 fi
				 cd /sdcard/Whatsapp/Media/.Statuses
	 ...
	 telegram | "join on telegram" ) echo "opening telegram"
         ...
         update | upgrade | "check version" ) version=$(cd && cd Emily && cat .version)
	 					...
	 					if [[ ! -d Emily ]]; then
							...
							cd Emily
							apt get update && upgarde && pkg install git && git clone https://github.com/naveen1749/Emily
							sleep 03
							check_version=$(cat .version)
								if [[ $check_version -ge $version ]]; then 
					      				cd 
									rm -rf Emily
									...
									sleep 02
										if [[ $check_version == $version ]]; then
						 					echo " successfullly updated. enjoy new feauchers. :) "
